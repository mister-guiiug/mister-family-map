...
test('@critical le profil offre la mise à jour forcée, le code source, le café et les autres apps', async ({
  ...
  // La grille des autres apps, alimentée par le catalogue du socle et groupée
  // par catégorie (`groupBy`). Depuis le socle 6.6.0 les groupes naissent
  // DÉPLIÉS : le clic sur le `summary` qui figurait ici les REFERMERAIT, et
  // l'assertion suivante tomberait — le remède d'hier est devenu le défaut.
